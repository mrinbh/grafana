...
const QueryHeader: React.FC<Props> = ({
  ...
  const onQueryModeChange = ({ value }: SelectableValue<CloudWatchQueryMode>) => {
    if (value && value !== queryMode) {
      let expression = isCloudWatchAnnotationQuery(query) ? undefined : query.expression;
      // If the mode is changing to logs mode and the expression is empty, use the default logs query string
      if ((value === 'Logs' && !expression) || expression === '') {
